...
export default function AdminProductsPage() {
  ...
  async function loadProducts() {
    ...
    const sellerIds = Array.from(
      new Set(list.map((p) => p.seller_id).filter((id): id is string => !!id))
    );
  ...
  return (
    <div>
      <h1 className="mb-1 font-display text-2xl font-bold text-mist">Kelola Produk</h1>
      <p className="mb-6 text-sm text-mist/60">
        Menampilkan semua produk, termasuk milik merchant. Form di bawah untuk
        menambah produk toko resmi (tanpa pemilik merchant). Gunakan tombol
        &quot;Hapus&quot; untuk menghapus produk mana pun, termasuk produk merchant
        yang melanggar aturan atau tidak pantas.
      </p>
            ...
            <div key={p.id} className="card flex items-center gap-4 p-4">
              <div className="flex-1">
                ...
                <p className="mt-1 text-xs text-mist/40">
                  Pemilik:{" "}
                  {p.seller_id
                    ? sellerNames[p.seller_id] || p.seller_id
                    : "Toko Resmi (Admin)"}
                </p>
              </div>
