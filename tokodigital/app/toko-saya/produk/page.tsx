...
// tambahkan suffix acak agar slug antar merchant tidak bentrok
function uniqueSlug(text: string) {
  const rand = Math.random().toString(36).slice(2, 6);
  return `${slugify(text)}-${rand}`;
}

export default function TokoSayaProdukPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (form.id) {
      // update: tidak mengubah slug supaya link produk lama tetap valid
      const payload = {
        name: form.name,
      ...
      const { error } = await supabase
        ...
        .update(payload)
        .eq("id", form.id)
        .eq("seller_id", userId); // jaga-jaga, hanya boleh update produk sendiri
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from("products").delete().eq("id", id).eq("seller_id", userId);
    loadProducts(userId);
  }

  return (
    <div>
      <h1 className="mb-1 font-display text-2xl font-bold text-mist">Produk Saya</h1>
      <p className="mb-6 text-sm text-mist/60">
        Kelola produk toko Anda sendiri. Produk milik merchant lain tidak bisa
        Anda ubah atau hapus.
      </p>
      ...
      {loading ? (
        ...
        <p className="text-mist/60">Anda belum punya produk. Tambahkan produk pertama Anda di atas.</p>
      ) : (
