  ...
  return (
    <div>
      ...
      <p className="mb-6 text-sm text-mist/60">
        Daftar pesanan dari produk toko Anda. Klaim saldo dapat diajukan setelah
        pesanan berstatus &quot;Selesai&quot; (sudah dicek dan dikonfirmasi oleh admin).
      </p>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {o.status !== "selesai" && (
                      <p className="text-xs text-mist/40">
                        Saldo bisa diklaim setelah pesanan selesai
                      </p>
                    )}
                    {o.status === "selesai" && !claim && (
                      <button
                        onClick={() => handleClaim(o)}
                        disabled={claiming === o.id}
                        className="btn-primary !py-1.5 !px-3 text-sm"
                      >
                        {claiming === o.id ? "Mengajukan..." : "Klaim Saldo"}
                      </button>
                    )}
                    {claim && (
                      <span
