...
-- trigger: saat admin menyetujui klaim saldo -> pesanan otomatis jadi 'selesai'
-- DAN saldo merchant otomatis bertambah (satu langkah persetujuan)
...
begin
  if new.status = 'approved' and old.status is distinct from 'approved' then
    ...
    update orders set status = 'selesai', updated_at = now() where id = new.order_id;
    new.reviewed_at = now();
...
-- balance_claims: seller boleh menandai pesanan miliknya (yang sudah diverifikasi
-- pembayarannya oleh admin / status 'diproses') sebagai selesai untuk diajukan
-- ke admin; jumlah harus sama persis dengan total pesanan tsb. Setelah admin
-- menyetujui, trigger di atas otomatis mengubah status pesanan jadi 'selesai'
-- dan menambah saldo merchant.
