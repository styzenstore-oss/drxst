...
// untuk pesanan produk merchant, status "selesai" hanya boleh terjadi lewat alur
// merchant menandai selesai -> admin menyetujui di halaman Konfirmasi Selesai
// (supaya saldo merchant ikut otomatis terisi). Admin tidak set manual di sini.
const STATUS_OPTIONS_MERCHANT_ORDER = STATUS_OPTIONS.filter((s) => s !== "selesai");
              ...
              <div className="flex flex-wrap items-start justify-between gap-2">
                ...
                <div className="flex flex-col items-end gap-1">
                  <select
                    ...
                    disabled={o.status === "selesai" && !!o.seller_id}
                  >
                    {(o.seller_id ? STATUS_OPTIONS_MERCHANT_ORDER : STATUS_OPTIONS).map(
                      (s) => (
                    ...
                    {o.status === "selesai" && o.seller_id && (
                      <option value="selesai">{ORDER_STATUS_LABEL.selesai}</option>
                    )}
                  </select>
                  {o.seller_id && o.status !== "selesai" && (
                    <p className="max-w-[220px] text-right text-[11px] text-mist/40">
                      Produk merchant — status &quot;Selesai&quot; diisi otomatis lewat
                      persetujuan di halaman Konfirmasi Selesai.
                    </p>
                  )}
                </div>
              </div>
