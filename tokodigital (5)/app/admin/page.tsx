...
export default function AdminDashboard() {
    ...
    async function load() {
      const [
        ...
        supabase
          ...
          .select("*", { count: "exact", head: true })
          .eq("status", "pending"),
        supabase.from("profiles").select("balance").eq("role", "seller"),
      ]);
      ...
      const outstandingBalance = (allProfiles || []).reduce(
        (sum: number, p: { balance: number }) => sum + Number(p.balance || 0),
        0
      );
  ...
  const cards = [
    ...
    { label: "Konfirmasi Selesai Menunggu", value: stats.pendingClaims },
    { label: "Permintaan Pencairan Menunggu", value: stats.pendingWithdrawals },
    { label: "Total Saldo Merchant (Belum Cair)", value: formatRupiah(stats.outstandingBalance) },
  ];
